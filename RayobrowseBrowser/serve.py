...
# 1. Запуск внутреннего демона Rayobrowse на внутреннем порту 9223
env = os.environ.copy()
...
daemon_cmd = ["/entrypoint.sh"] if os.path.exists("/entrypoint.sh") else ["/usr/local/bin/entrypoint.sh"]
...
if not daemon_ready:
    print("[rayobrowse] ОШИБКА: Демон не поднялся за 40 секунд!", flush=True)
    daemon_proc.kill()
    sys.exit(1)

# 2. Инициализация браузерной сессии с расширениями
ext_paths = []
...
print(f"[rayobrowse] Запрос постоянного браузера через /connect...", flush=True)
resp = httpx.get(DAEMON_CONNECT_URL, params=params, timeout=60)
resp.raise_for_status()

UPSTREAM_WS_URL = resp.text.strip()
print(f"[rayobrowse] Браузер создан, WebSocket CDP: {UPSTREAM_WS_URL}", flush=True)
...
async def main_server():
    ...
    await site.start()
    print(f"[rayobrowse] CDP Bridge успешно запущен на 0.0.0.0:{EXTERNAL_PORT}", flush=True)
